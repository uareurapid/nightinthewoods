...
#region Component Segments
	...
	void OnGUI () {
		...
		// Show Scenes window
		GUI.Window(3, new Rect(5, Screen.height-130, 210, 125), DemoScenesWindow, "Demo Scenes");
...
#region Functions
	...
	void HelpWindow(int id)
	{
		if(m_FirstPerson.activeSelf==true)
		{
			...
			GUI.Label(new Rect(12, 75, 240, 20), "E: Switch to Orbit Camera");
		}
	...
	{
		string[] DemoScenesName = {"Arctic", "Grassland", "Volcano", "Wasteland"};
		for(int i=0;i<DemoScenesName.Length;i++)
		{
			...
			if(GUI.Button(new Rect(10, 25*(i+1), 190, 20), DemoScenesName[i]))
			{
	...
	{
		GUI.Label(new Rect(15, 25, 240, 20), "First Ground 1.3.0");
